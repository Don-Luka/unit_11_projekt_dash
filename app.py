    ...
    def transation_init():
        ...
        for filename in os.listdir(src):
            transactions = pd.concat([transactions, 
                                    pd.read_csv(os.path.join(src, filename), 
                                                index_col=0)])
        def convert_dates(x):
            try:
...
# ogólny callback - wybór zakładki
...
## tab1 callbacks
@app.callback(Output(component_id='bar-sales',component_property='figure'),
...
## tab3 callbacks
@app.callback(Output('pie-store-weekday','figure'),
            [Input('store_radio','value')])
def tab3_barh_prod_subcat(chosen_storetype):

    grouped = df.merged[(df.merged['total_amt']>0)][['total_amt']]\
        .groupby([df.merged['Store_type'],df.merged['tran_date'].dt.day_name().map({'Monday': 'poniedziałek',
                                                                    'Tuesday': 'wtorek',
                                                                    'Wednesday': 'Środa',
                                                                    'Thursday': 'czwartek',
                                                                    'Friday': 'piątek',
                                                                    ...
                                                                    'Sunday': 'niedziela'})])\
                                                                    .sum().unstack()
    grouped.columns = [''.join(col).removeprefix('total_amt') for col in grouped.columns.values]

    fig = go.Figure(data=[go.Pie(labels=grouped.loc[grouped.index==chosen_storetype].columns,
                             values=grouped.loc[grouped.index==chosen_storetype].values[0])],
